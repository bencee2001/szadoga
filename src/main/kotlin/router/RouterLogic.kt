...
object RouterLogic: RouterInterface {

    private val produceUnits = listOf(UnitType.INVERTER, UnitType.ENGINE)

    override suspend fun getTargetByUnits(park: Park, target: Double): Map<UnitType, Map<Int, Double>> {
        ...
        val targetByUnitType = getTargetByUnitType(targetVal, park)
        targetByUnitId[UnitType.INVERTER] = getInverterTargets(park, targetByUnitType[UnitType.INVERTER] ?: targetVal)
        targetByUnitId[UnitType.LOADBANK] = getLoadbankTargets(park, targetDiff.times(-1))
        targetByUnitId[UnitType.ENGINE] = getEngineTargets(park, targetByUnitType[UnitType.ENGINE] ?: targetVal)
    ...
    private fun getSumRatedAcPowerByUnitType(park: Park): Map<UnitType, Double>{
        val powerByUnitType = mutableMapOf<UnitType, Double>()
        produceUnits.forEach {
            powerByUnitType[it] = park.unitList.filter { unit -> unit.type == it }.sumOf { unit -> unit.ratedAcPower }
        }
        return powerByUnitType
    }

    private fun getTargetByUnitType(target: Double, park: Park): Map<UnitType, Double>{
        val powerByUnitType = getSumRatedAcPowerByUnitType(park)
        val sum = powerByUnitType.values.sum()
        return powerByUnitType.mapValues { target.div(sum).times(it.value) }
    }
